...
LinkedList MIProcessQuery(MemIndex index, char *query[], uint8_t qlen) {
  ...
  // create a HashIterator to find each DocID
  HTIter it = HashTableMakeIterator(wds->docIDs);
  Verify333(it != NULL);
  while (!HTIteratorPastEnd(it)) {
    res = HTIteratorGet(it, &kv);
    Verify333(res == 1);
    SearchResult *srp = (SearchResult *) malloc(sizeof(SearchResult));
    Verify333(srp != NULL);
    srp->docid = kv.key;
    srp->rank = NumElementsInLinkedList((LinkedList) kv.value);
    Verify333(AppendLinkedList(retlist, (void *) srp));
    res = HTIteratorNext(it);
  }
  HTIteratorFree(it);
  ...
  for (i = 1; i < qlen; i++) {
    ...
    for (j = 0; j < ne; j++) {
      SearchResult* srp;
      LLIteratorGetPayload(llit, (void **) &srp);
      res = LookupHashTable(wds->docIDs, srp->docid, &kv);
      Verify333(res != -1);
      if (res == 0) {
        Verify333(LLIteratorDelete(llit, &free));
      } else {
        srp->rank += NumElementsInLinkedList((LinkedList) kv.value);
        LLIteratorNext(llit);
      }
