...
static HWSize_t WriteDocPositionListFn(FILE *f,
  ...
  for (i = 0; i < num_pos_ho; i++) {
    ...
    // MISSING:
    LLIteratorGetPayload(it, reinterpret_cast<LLPayload_t *> (&position));
...
static HWSize_t WriteWordDocSetFn(FILE *f,
  ...
  // Calculate and return the total amount of data written.
  // MISSING (fix this return value):
    return (htlen_ho + wordlen_ho + sizeof(header));
}
...
static HWSize_t WriteBucket(FILE *f,
  ...
  if (chainlen_ho > 0) {
    ...
    for (j = 0; j < chainlen_ho; j++) {
      ...
      // MISSING:
      element_position_rec eprec = {nextelpos};
      eprec.toDiskFormat();
      ...
      res = fwrite(&eprec, sizeof(eprec), 1, f);
      if (res != 1) {
        LLIteratorFree(it);
        return 0;
      }
